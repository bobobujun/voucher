...
Page({
  ...
  checkBig(val) {
    ...
    if (!this.data.orderAmount&&dataItem.merchantProductVoucher[0].rechargeAmt!=null&&dataItem.merchantProductVoucher[0].rechargeAmt!=0) {//没有输入订单金额且优惠券金额不为0时提示
      wx.showToast({
        title: "请先选择订单金额",
        duration: 2000,
        icon: 'none'
    ...
    if (totalAmount>this.data.orderAmount) {
      wx.showToast({
  ...
  changeStep(val) {
    ...
    if (val.currentTarget.dataset.itemdata.checked) {
      ...
      if (temTotalAmount>=Number(this.data.orderAmount)&&dataItem.merchantProductVoucher[0].rechargeAmt!=null&&dataItem.merchantProductVoucher[0].rechargeAmt!=0) {
        wx.showToast({
          title: "您选中的优惠券已大于订单金额",
          duration: 2000,
          icon: 'none'
